...
public class GraphLib {
	...
	static int[] par = new int[MAX], d = new int[MAX], inq = new int[MAX]; // par[source] = source;
	// Labelling function
	...
	static int[] id = new int[MAX], pre = new int[MAX], low = new int[MAX], ord = new int[MAX];
	static int cnt, sCnt, bCnt;
	
	static int[] pred = new int[MAX], mins = new int[MAX], parent = new int[MAX];
	...
	static boolean isStronglyReachable(int u, int v) {
		// run strongComponentsTarjan first
		return id[u] == id[v];
	}
	
	static boolean[][] transitiveClosure(int N) { // O(E + V^2 + X^3) - X = strong components
		int i, j, k, M;
		...
		// compute transitive closure of the kernel DAG
		for (k = 0; k < M; k++)
			for (i = 0; i < M; i++)
				for (j = 0; j < M; j++)
	...
	static boolean dfs(int i) {
		...
		vis[i] = color;
		for (int j = 0; j < n; j++) {
			if (edge(i, j) && match[i] != j && (match[MAX + j] == -1 || dfs(match[MAX + j]))) {
				match[i] = j;
				match[MAX + j] = i;
				return true;
			}
	...
	static boolean dijkstraSparse(int n, int s, int t) {
		...
		par[s] = n;
		while (qs != 0) {
	        // get the minimum from q and bubble cur
	        int u = q[0]; inq[u] = -1;
	        q[0] = q[--qs];
	        if (qs != 0)
	        	inq[q[0]] = 0;
	        for (i = 0, j = 2*i + 1; j < qs; i = j, j = 2*i + 1 ) {
	            ...
	            tmp = inq[q[i]]; inq[q[i]] = inq[q[j]]; inq[q[j]] = tmp;
	        }
	...
	static int maxFlowMinCostSparse(int n, int s, int t) {
	    ...
	    while( dijkstraSparse( n, s, t ) ) {
	        ...
	        for( int v = t, u = par[v]; v != s; u = par[v = u] )
	            bot = Math.min(bot, fnet[v][u] != 0? fnet[v][u] : ( cap[u][v] - fnet[u][v] ));
	...
	// fcost es el costo minimo del fnet
	int maxFlowMinCost(int n, int s, int t) { // O(n^2*flow  <?  n^3*fcost)
	    // build the adjacency list
		Arrays.fill(deg, 0);
		int i, j, v, bot, u;
	    for (i = 0; i < n; i++)
	...
	static int dinic(int n, int s, int t) { // O(V^2 + E)
		// don't forget to include both u->v and v->u
	    int flow = 0;
	    int i, j, u, v, z, bot;
	    Arrays.fill(deg, 0, n, 0);
	    for (i = 0; i < n; i++)
	...
	static int[] dijkstra2(int s, int N) { // O(m * log(n))
		final int NN = N*N;
		int qs, t;
	...
	static int bipartiteMatching() { // O(m n)
	...
	static int prim(int N) {
		int i, j, cost = 0;
		set.clear(); mst.clear();
		set.add(new Edge(-1, 0, 0)); // start from vertex 0
		for (i = 0; i < N; i++) {
			iter = set.iterator();
			do {
				if (! iter.hasNext())
